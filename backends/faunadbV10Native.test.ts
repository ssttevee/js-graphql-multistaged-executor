...
const WrappedStringType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    restaged: {
      type: GraphQLString,
      resolve: async (src) => fql`${await src} + ""`,
    },
    recurseDeferred: {
...
const QueryType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    helloDeferred: {
      ...
      resolve: (_src, args) => fql`"Hello " + ${args.name}`,
    },
    ...
    helloAllDeferred: {
      ...
      resolve: (_src, args) =>
        fql`${args.names}.map((name) => "Hello " + name)`,
    },
    helloAllDeferredInterface: {
      ...
      resolve: (_src, args) => fql`${args.names}.map((name) => {
        __typename: "SimpleString",
        string: "Hello " + name,
      })`,
    },
