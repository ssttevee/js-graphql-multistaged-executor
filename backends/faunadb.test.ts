...
import createExecutorBackend from "./faunadb";
...
const execute = createExecuteFn(createExecutorBackend({
  endpoint: "http://localhost:8443",
  secret: "secret",
  // fetch: fetch,
}));

async function isFaunaDBReady() {
  try {
...
beforeAll(async () => {
  if (await isFaunaDBReady()) {
    return;
  }

  const p = await promisify(exec)("docker run -p 8443:8443 -d --rm fauna/faunadb");

  containerId = p.stdout.trim();

  console.log("Waiting for FaunaDB to start...");

  while (!await isFaunaDBReady()) {
...
test("helloWrapped.json", async () => {
  const result = await execute({
    schema,
    document: parse(
      `query { helloWrapped(name: "world") { json } }`,
    ),
    rootValue: null,
  });
  if (result.errors?.length) {
...
test("helloAll", async () => {
  const result = await execute({
    schema,
    document: parse(
      `query { helloAll(names: ["world", "jim"]) }`,
    ),
    rootValue: null,
  });
  if (result.errors?.length) {
...
test("helloDeferred", async () => {
  const result = await execute({
    schema,
    document: parse(
      `query { helloDeferred(name: "world")  }`,
    ),
    rootValue: null,
  });
  if (result.errors?.length) {
...
test("helloAllDeferred", async () => {
  const result = await execute({
    schema,
    document: parse(
      `query { helloAllDeferred(names: ["world", "jim"])  }`,
    ),
    rootValue: null,
  });
  if (result.errors?.length) {
...
test("helloDeferredWrapped.json", async () => {
  const result = await execute({
    schema,
    document: parse(
      `query { helloDeferredWrapped(name: "world") { json } }`,
    ),
    rootValue: null,
  });
  if (result.errors?.length) {
